...
osg.State = function () {
    ...
    this.shaderGeneratorAttributes.attributeKeys = [];
...
osg.State.prototype = {
    ...
    collectUniformsAppliedAttributes: function(shaderAttributeMap) {
        ...
        var attribute;
        var uniform;
        for (var i = 0, l = shaderAttributeMap.attributeKeys.length; i < l; i++) {
            var key = shaderAttributeMap.attributeKeys[i];
            attribute = shaderAttributeMap[key];
            if (attribute.getOrCreateUniforms !== undefined) {
                var uniformsFromAttribute = attribute.getOrCreateUniforms();
                for (var j = 0, m = uniformsFromAttribute.uniformKeys.length; j < m; j++) {
                    var uniformKey =  uniformsFromAttribute.uniformKeys[j];
                    uniform = uniformsFromAttribute[uniformKey];
                    uniforms[uniform.name] = uniform;
                }
            }
        }
    ...
    collectUniformsAppliedAttributesOld: function(shaderAttributeMap) {
        var uniforms = {};
        var attribute;
        var uniform;
        for (var key in shaderAttributeMap) {
            attribute = shaderAttributeMap[key];
            if (attribute.getOrCreateUniforms !== undefined) {
                var uniformsFromAttribute = attribute.getOrCreateUniforms();
                for (var uniformKey in uniformsFromAttribute) {
                    uniform = uniformsFromAttribute[uniformKey];
                    uniforms[uniform.name] = uniform;
                }
            }
    ...
    applyUniformList: function(uniformMap, uniformList) {
        ...
        for (var i = 0, l = programUniforms.uniformKeys.length; i < l; i++) {
            ...
            location = programUniforms[uniformKey].location;
        ...
        for (var i = 0, l = attributeMap.attributeKeys.length; i < l; i++) {
            ...
            if (attributeStack.lastApplied !== attribute) {
                if (attribute.apply) {
                    ...
                    this.shaderGeneratorAttributes[name] = attribute;
                    if (this.shaderGeneratorAttributes.attributeKeys.indexOf(name) === -1) {
                        this.shaderGeneratorAttributes.attributeKeys.push(name);
                    }
                }
    ...
    pushUniformsListOld: function(uniformMap, uniformList) {
        var name;
        var uniform;
        for ( var key in uniformList) {
    ...
    popUniformsListOld: function(uniformMap, uniformList) {
        var uniform;
        for (var key in uniformList) {
            uniformMap[key].pop();
        }
    ...
    applyTextureAttributeMapListOld: function(textureAttributesMapList) {
        ...
        for (var textureUnit = 0, l = textureAttributesMapList.length; textureUnit < l; textureUnit++) {
            ...
            for (var key in textureAttributeMap) {
                attributeStack = textureAttributeMap[key];
                if (attributeStack === undefined) {
                    continue;
                }
    ...
    pushAttributeMapOld: function(attributeMap,  attributeList) {
        var attributeStack;
        for (var type in attributeList) {
    ...
    popAttributeMapOld: function(attributeMap,  attributeList) {
        var attributeStack;
        for (var type in attributeList) {
            attributeStack = attributeMap[type];
            attributeStack.pop();
