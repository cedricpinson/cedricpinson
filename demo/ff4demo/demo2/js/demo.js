    ...
    releaseButton: function() {
        ...
        if (hit !== undefined && this.pushHit !== undefined && this.pushHit.name !== undefined && this.pushHit.name === hit.name) {
            ...
            if (this.currentSelectedItem === item) { // go into
                if (item.getSubGraph() !== undefined) { // if it's a directory
...
function CameraAutoCallback(view) { this.view = view;}

CameraAutoCallback.prototype = {
    update: function(node, nv) {
        var m = this.view.getViewMatrix();
        minv = osg.Matrix.inverse(m);
        node.setMatrix(minv);
...
//    twitter.setProjectionMatrix(osg.Matrix.makeOrtho(-ratio, ratio, -1.0, 1.0, -1,1));
    ...
    if (false ) {
        ...
        Cube = new osg.MatrixTransform();
        Cube.addChild(osg.ParseSceneGraph(getBuilding()));
        Cube.setMatrix(osg.Matrix.makeScale(4,4,4));
    }
